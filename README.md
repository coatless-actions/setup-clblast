...
| `platform-name` | `CL_PLATFORM_NAME` of the platform the GEMM ran on. Empty when no platform was found, and when `verify` is `false`. |
| `device-name` | `CL_DEVICE_NAME` of the device the GEMM ran on. Empty when no device was found, and when `verify` is `false`. |
| `clblast-module` | Fully resolved path of the CLBlast shared library the verifier was actually **bound to** — the `.so`/`.dylib`/`.dll` the loader mapped. On Linux and macOS that is the versioned file behind the symlink `clblast-library` names; on Windows it is the DLL rather than the import library. `unknown` when it could not be determined; empty when `verify` is `false`. |
| `opencl-module` | Fully resolved path of the OpenCL ICD loader the verifier was actually bound to. Compare against `setup-opencl`'s `OpenCL_LIBRARY`. `unknown` when it could not be determined; empty when `verify` is `false`. |

`clblast-module` and `opencl-module` answer a question none of the other outputs do: **where** the verified GEMM ran. A correct GEMM is not by itself evidence that it used the library this action installed. On Linux the apt package's link flags are a bare `-lclblast` with no `-L`, so any `libclblast` on the default search path satisfies the link; on Windows the PE search order reaches the application directory and `System32` before it reaches `PATH`, and Windows ships its own `OpenCL.dll` in `System32`. Either substitution produces perfectly correct numbers. If your job depends on the library it asked for, compare these two paths in resolved form (`realpath`, or `cygpath -m -a` folded to lower case on Windows) against `clblast-library` and `OpenCL_LIBRARY` — symlink chains, macOS `/private` prefixes and drive-letter case make a raw string comparison useless. `tests/assert-module-identity.sh` in this repository does exactly that and is usable as a reference.
